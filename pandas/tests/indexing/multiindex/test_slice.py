...
class TestMultiIndexSlicers:
    ...
    def test_multiindex_slicers_datetimelike(self):
        ...
        freq = [1, 2]
        index = MultiIndex.from_product([dates, freq],
                                        names=["date", "frequency"])
        ...
        result = df.loc[(
            slice(Timestamp("2012-01-01 12:12:12"
        ...
        result = df.loc[(
            idx[Timestamp("2012-01-01 12:12:12"
                          ):Timestamp("2012-01-03 12:12:12")],
            idx[1:1],
        ),
            slice("A", "B"), ]
    ...
    def test_per_axis_per_level_doc_examples(self):
        ...
        expected = df.loc[[(
            ...
            c,
            d,
        ) for a, b, c, d in df.index.values if c in ("C1", "C3")]]
        tm.assert_frame_equal(result, expected)
    ...
    def test_loc_axis_single_level_multi_col_indexing_multiindex_col_df(self):
        ...
        df = DataFrame(
            ...
            columns=MultiIndex.from_product([["a1", "a2", "a3"],
                                             ["b1", "b2", "b3"]]),
        )
    ...
    def test_loc_slice_negative_stepsize(self):
        ...
        expected = DataFrame([[3, 4], [1, 2]],
                             index=MultiIndex.from_tuples([("a", 1),
                                                           ("a", 0)]))
        tm.assert_frame_equal(result, expected)
